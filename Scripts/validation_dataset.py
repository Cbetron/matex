...
def trainingset_validation(path, toexclude=None, usefeatures=None):
    ...
    for i in range(100):
        ...
        while True:
            try:
                ...
                new_profile, ergs = validation.profile_validation(tmp_profile, valset, rating=rating, multitest=3,
                                                                  toexclude=toexclude)
                for feature in featurelist:
    ...
    with contextlib.closing(open("traininset_validation.txt", "w")) as f:
        f.write(
...
exclude = input("Exclude Features?: ")
use = input("use Features?: ").split()
trainingset_validation(path, exclude, use)
